...
const XRay = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Conditions: category = 'Radiology', radiology_type = 'X-Ray', planned3 IS NOT NULL, actual3 IS NULL, payment_status IS NOT NULL
      ...
      // Load completed X-Ray records (actual3 IS NOT NULL)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const fileName = `xray_${selectedRecord.id}_${Date.now()}.${fileExt}`;
  ...
  const handleViewReport = (reportUrl) => {
    if (!reportUrl) {
      showNotification('No X-Ray report available', 'info');
      return;
    }
    ...
    return records.filter(record => {
      ...
      if (selectedDate) {
        if (!record.planned3) {
          ...
          // Extract YYYY-MM-DD part from planned3 which is likely in timestamp or similar format
          // The filters use input type='date' which is YYYY-MM-DD.
          // We can check substring or use Date object.
    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex-none bg-white border-b border-gray-200 shrink-0 shadow-sm z-10">
        <div className="px-4 py-2 sm:px-6 border-b border-gray-100">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-xl font-bold text-gray-900 sm:text-2xl lg:text-3xl">X-Ray Management</h1>
              <p className="hidden mt-0.5 text-xs text-gray-600 sm:block">Process X-Ray reports and manage records</p>
            </div>
            <button
              onClick={loadData}
        ...
        <div className="px-3 sm:px-4 py-2">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-2">
            {[
              { label: 'Total X-Ray', val: totalRecords, color: 'text-green-600', bg: 'bg-green-50', border: 'border-green-100' },
                    ...
                    {filteredHistoryRecords.length > 0 ? (
                      filteredHistoryRecords.map((record) => (
                        <tr key={record.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-gray-900 max-w-[350px] whitespace-normal break-words text-center">{record.xrayRemarks}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
        ...
        <div className="overflow-y-auto fixed inset-0 z-50 flex justify-center items-center p-4 bg-black bg-opacity-50">
          <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-xl">
            <div className="sticky top-0 z-10 flex justify-between items-center p-4 bg-white border-b border-gray-200 md:p-6">
              <h2 className="text-xl font-bold text-gray-900 md:text-2xl">Process X-Ray Report</h2>
              <button
                onClick={() => {
                  setShowModal(false);
                  resetForm();
      ...
      {showViewModal && viewingRecord && (
        <div className="overflow-y-auto fixed inset-0 z-50 flex justify-center items-center p-4 bg-black bg-opacity-50">
          <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-xl">
            <div className="sticky top-0 z-10 flex justify-between items-center p-4 bg-white border-b border-gray-200 md:p-6">
              <h2 className="text-xl font-bold text-gray-900 md:text-2xl">X-Ray Report Details</h2>
              <button
            ...
            <div className="p-4 md:p-6 space-y-6">
                ...
                <div className="p-2 border rounded-lg">
                  {viewingRecord.xrayReport ? (
                    <div className="relative">
                      {viewingRecord.xrayReport.toLowerCase().includes('.pdf') ? (
                        <div className="relative group">
                          <iframe
                            src={viewingRecord.xrayReport}
                            className="w-full h-96 rounded border border-gray-200"
                            title="X-Ray Report PDF"
                          />
                          <button
                            onClick={() => handleViewReport(viewingRecord.xrayReport)}
              ...
              <div>
                ...
                <div className="p-3 bg-gray-50 rounded-lg text-sm text-gray-700 whitespace-pre-wrap">
                  {viewingRecord.xrayRemarks || 'No remarks added'}
                </div>
              </div>
            </div>
          </div>
