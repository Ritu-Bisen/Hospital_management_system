...
const CTScan = () => {
  ...
  useEffect(() => {
    ...
    // Reload data when window gains focus (user switches back to tab)
    const handleFocus = () => {
      console.log('CT-Scan page focused - reloading data');
      loadData();
    };
  ...
  const loadData = () => {
    try {
      const storedHistory = localStorage.getItem('ctScanHistory');
      const paymentHistory = localStorage.getItem('paymentHistory');
      
      // Load CT-Scan history
      const existingHistory = storedHistory ? JSON.parse(storedHistory) : [];
      setHistoryRecords(existingHistory);

      // Load pending from payment history where category is Radiology and radiologyType is CT-Scan
      if (paymentHistory) {
        const paymentRecords = JSON.parse(paymentHistory);
        
        // Get IDs of records already in CT-Scan history
        const processedIds = existingHistory.map(record => record.ctScanId || record.paymentId || record.adviceId);
        
        console.log('=== CT-Scan Page Debug ===');
        console.log('Payment History Records:', paymentRecords.length);
        console.log('Processed CT-Scan IDs:', processedIds);
        console.log('All Payment Records:', paymentRecords);
        
        // Filter for CT-Scan records that haven't been processed
        // Handle both "CT-Scan" and "CT-scan" (case insensitive)
        const newPending = paymentRecords
          .filter(record => {
            const radiologyTypeLower = record.radiologyType?.toLowerCase();
            const isCTScan = record.category === 'Radiology' && 
                           (radiologyTypeLower === 'ct-scan' || radiologyTypeLower === 'ct scan');
            ...
            return isCTScan && notProcessed;
          })
          .map(record => ({
            ...record,
            ctScanId: record.paymentId || record.adviceId || Date.now() + Math.random()
          }));
        
        console.log('New Pending CT-Scan Records:', newPending.length);
        console.log('Pending Records:', newPending);
        
        setPendingRecords(newPending);
        localStorage.setItem('ctScanPending', JSON.stringify(newPending));
      } else {
  ...
  const saveToStorage = (pending, history) => {
    try {
      localStorage.setItem('ctScanPending', JSON.stringify(pending));
      localStorage.setItem('ctScanHistory', JSON.stringify(history));
    } catch (error) {
  ...
  const handleSubmit = () => {
    ...
    // Add to history
    const updatedHistory = [ctScanRecord, ...historyRecords];
    
    // Remove from pending
    const updatedPending = pendingRecords.filter(p => p.ctScanId !== selectedRecord.ctScanId);
  ...
  return (
    <div className="p-3 space-y-4 md:p-6 bg-gray-50 min-h-screen">
      <div className="flex flex-col gap-3 justify-between items-start sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 md:text-3xl">CT-Scan Management</h1>
          <p className="mt-1 text-sm text-gray-600">Process CT-Scan reports and manage records</p>
        </div>
        <button
          onClick={loadData}
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <div className="p-4 bg-white rounded-lg border border-green-200 shadow-sm">
          <div className="flex flex-col">
            <span className="text-xs font-medium text-gray-600 uppercase">Total CT-Scan Records</span>
      ...
      {activeTab === 'pending' && (
        <>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {pendingRecords.length > 0 ? (
                  pendingRecords.map((record) => (
                    <tr key={record.ctScanId} className="hover:bg-gray-50">
                  ...
                  <tr>
                    <td colSpan="14" className="px-4 py-8 text-center text-gray-500">
                      ...
                      <p className="text-sm text-gray-500 mt-1">CT-Scan records from payment history will appear here</p>
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="space-y-3 md:hidden">
            {pendingRecords.length > 0 ? (
              pendingRecords.map((record) => (
                <div key={record.ctScanId} className="p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
              ...
              <div className="p-8 text-center bg-white rounded-lg border border-gray-200 shadow-sm">
                ...
                <p className="text-xs text-gray-500 mt-1">CT-Scan records from payment history will appear here</p>
              </div>
            )}
          </div>
        </>
          ...
          <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-xl">
            ...
            <div className="p-4 md:p-6 space-y-6">
              ...
              <div className="p-4 bg-indigo-50 rounded-lg border border-indigo-200">
                <h3 className="mb-3 text-sm font-semibold text-gray-900">CT-Scan Details</h3>
