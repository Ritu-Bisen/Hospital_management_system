...
const CTScan = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Conditions: category = 'Radiology', radiology_type = 'CT-Scan', planned1 IS NOT NULL, actual1 IS NULL, payment_status IS NOT NULL
      ...
      // Load completed CT-Scan records (ctscan_report IS NOT NULL)
      ...
      const formattedHistory = historyData.map(record => {
        return {
          ...
          ctscanCompletedDate: record.ctscan_completed_date,
          planned1: record.planned1,
          actual1: record.actual1,
          paymentStatus: record.payment_status,
          ctScanId: record.id,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const fileName = `ctscan_${selectedRecord.id}_${Date.now()}.jpg`;
  ...
  const handleViewImage = (imageUrl) => {
    if (!imageUrl) {
      alert('No CT-Scan report available');
      return;
    }
                ...
                {historyRecords.length > 0 ? (
                  historyRecords.map((record) => (
                    <tr key={record.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900 max-w-xs truncate">{record.ctscanRemarks}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
              ...
              <div className="p-4 bg-indigo-50 rounded-lg border border-indigo-200">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="pt-3 border-t border-indigo-300">
                    <span className="text-gray-600">Remarks:</span>
                    <div className="font-medium text-gray-900 mt-1 whitespace-pre-wrap">{viewingRecord.ctscanRemarks}</div>
                  </div>

                  <div>
                    <span className="text-gray-600">Processed Date:</span>
                    <div className="font-medium text-gray-900 mt-1">
                      {viewingRecord.ctscanCompletedDate ? new Date(viewingRecord.ctscanCompletedDate).toLocaleString() : 'N/A'}
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  {viewingRecord.ctscanReport ? (
                    <>
                      ...
                      />
                      <button
                        onClick={() => handleViewImage(viewingRecord.ctscanReport)}
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-600">No CT-Scan report available</p>
                    </div>
                  )}
                </div>
              </div>
