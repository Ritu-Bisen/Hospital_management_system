...
const AssignTask = () => {
    ...
    const [dropdownStates, setDropdownStates] = useState({
        shiftA: { showNurseDropdown: false, nurseSearch: '' },
        shiftB: { showNurseDropdown: false, nurseSearch: '' },
        shiftC: { showNurseDropdown: false, nurseSearch: '' }
    });
    ...
    useEffect(() => {
        const loadTasksFromSupabase = async () => {
            try {
                ...
                console.log("Supabase response for tasks:", data);
                ...
                if (data && data.length > 0) {
                    ...
                    console.log("Loaded tasks:", uniqueTasks);
                } else {
    ...
    const handleAddNewTask = async () => {
        ...
        try {
            ...
            showNotification('Error saving new task to database', 'error');
        } finally {
    ...
    // Load Nurses from all_staff table
    useEffect(() => {
        const loadNurses = async () => {
            try {
                ...
                if (data && data.length > 0) {
                    setNurses(data.map(staff => staff.name));
                } else {
                    setNurses([]);
                }
            } catch (error) {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            ...
            if (isOutsideAllDropdowns) {
                setDropdownStates({
                    shiftA: { ...dropdownStates.shiftA, showNurseDropdown: false },
                    shiftB: { ...dropdownStates.shiftB, showNurseDropdown: false },
                    shiftC: { ...dropdownStates.shiftC, showNurseDropdown: false }
                });
            }
        };
    ...
    // Filter nurses based on search for specific shift
    const getFilteredNurses = (shiftKey) => {
        const searchTerm = dropdownStates[shiftKey].nurseSearch.toLowerCase();
        return nurses.filter(nurse =>
            nurse.toLowerCase().includes(searchTerm)
        );
    };
    ...
    const handleShiftChange = (shift, field, value) => {
        ...
        if (field === 'assignNurse') {
            // Only close the dropdown for this specific shift
            handleDropdownStateChange(shift, 'showNurseDropdown', false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        Object.entries(shifts).forEach(([shiftKey, shiftData]) => {
            if (shiftData.assignNurse && !shiftData.startDate) {
                errors.push(`Start date is required for ${shiftKey.toUpperCase()}`);
            }
            if (shiftData.assignNurse && shiftData.tasks.some(t => !t.taskName.trim())) {
        ...
        try {
            ...
            for (const [shiftKey, shiftData] of Object.entries(shifts)) {
                if (!shiftData.assignNurse) continue; // Skip empty shifts
                        ...
                        .insert([
                            {
                                ...
                                assign_nurse: shiftData.assignNurse,
            ...
            showNotification('Error saving tasks to database', 'error');
        }
    };
    ...
    const shiftConfig = {
        shiftA: { label: 'Shift A', color: 'from-blue-50 to-blue-100', border: 'border-blue-200' },
        ...
        <div className="p-4 sm:p-6 bg-gray-50 min-h-screen">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-gray-600 mt-1">Assign nursing tasks for different shifts</p>
                </div>
                                ...
                                <div className="space-y-6">
                                            ...
                                            {loadingNurses ? (
                                                ...
                                                <>
                                                    <div className="relative">
                                                        <input
                                                            type="text"
                                                            value={shifts[shiftKey].assignNurse}
                                                            onChange={(e) => handleShiftChange(shiftKey, 'assignNurse', e.target.value)}
                                                            onFocus={() => {
                                                                // Close other dropdowns first
                                                                Object.keys(dropdownStates).forEach(key => {
                                                                    if (key !== shiftKey) {
                                                                        handleDropdownStateChange(key, 'showNurseDropdown', false);
                                                                    }
                                                                });
                                                                handleDropdownStateChange(shiftKey, 'showNurseDropdown', true);
                                                            }}
                                                    ...
                                                    {dropdownStates[shiftKey].showNurseDropdown && (
                                                        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                                            <div className="sticky top-0 bg-white p-2 border-b">
                                                                <div className="relative">
                                                                    ...
                                                                    <input
                                                                        type="text"
                                                                        value={dropdownStates[shiftKey].nurseSearch}
                                                                        onChange={(e) => handleDropdownStateChange(shiftKey, 'nurseSearch', e.target.value)}
                                                            ...
                                                            <div className="py-1">
                                                                {getFilteredNurses(shiftKey).length > 0 ? (
                                                                    getFilteredNurses(shiftKey).map((nurse, index) => (
                                                                        <div
                                                                            key={`${nurse}-${index}`}
                                                                            className="px-3 py-2 text-sm hover:bg-green-50 cursor-pointer"
                                                                            onClick={() => {
                                                                                handleShiftChange(shiftKey, 'assignNurse', nurse);
                                                                                handleDropdownStateChange(shiftKey, 'showNurseDropdown', false);
                                                                                handleDropdownStateChange(shiftKey, 'nurseSearch', '');
                                                                            }}
                                                                        >
                                                                            {nurse}
                                                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex justify-between items-center mt-2">
                                            <p className="text-xs text-gray-500">
                                                Select tasks from dropdown or add custom tasks
                                            </p>
                                            <p className="text-xs text-gray-500">
                        ...
                        <div className="p-6">
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    placeholder="e.g., Wound Dressing, Oxygen Therapy, etc."
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                >
                                    {savingNewTask ? (
                                        <>
                            ...
                            <p className="text-xs text-gray-500 mt-3 text-center">
                                This task will be saved to Supabase and available for future use
                            </p>
                        </div>
                    </div>
                </div>
