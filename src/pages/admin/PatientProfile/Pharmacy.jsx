...
import { ArrowLeft, Pill, Plus, X, Save, Trash2, Eye, Edit, CheckCircle } from 'lucide-react';
...
const StatusBadge = ({ status }) => {
  const getColors = () => {
    if (status === 'Completed' || status === 'Approved & Dispensed') return 'bg-green-100 text-green-700 border-green-300';
    if (status === 'Pending' || status === 'Pending Approval') return 'bg-yellow-100 text-yellow-700 border-yellow-300';
  ...
  const handleSubmit = () => {
    ...
    if (editMode && selectedIndent) {
      ...
      const newIndent = {
        ...
        uhidNumber: data.personalInfo.uhid || '',
        diagnosis: formData.diagnosis,
        requestTypes: { ...requestTypes },
        medicines: [...medicines],
        investigationAdvice: { ...investigationAdvice },
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">{data.personalInfo.name} - {data.personalInfo.uhid}</p>
          </div>
          <button
            onClick={() => {
              resetForm();
                ...
                {submittedIndents.length > 0 ? (
                  submittedIndents.map((indent) => (
                    <tr key={indent.indentNumber} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm">{indent.admissionNo}</td>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{selectedIndent.admissionNo}</p>
                  </div>
                  <div>
